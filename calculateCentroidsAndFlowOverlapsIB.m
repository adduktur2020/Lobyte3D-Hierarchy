function glob = calculateCentroidsAndFlowOverlapsIB(glob, depos)
...
% multiply this by the inverse of the normalised flow overlap area to give a final flow separation metric
    ...
    elev = zeros(1,glob.totalIterations);
    ...
    for t = 1:glob.totalIterations
        ...
        if pointCount > 0
            centroidX(t) = round(centroidX(t) / pointCount);
            centroidY(t) = round(centroidY(t) / pointCount);
            elev(t) = depos.elevation(centroidY(t),centroidX(t),t);
        end
    end
